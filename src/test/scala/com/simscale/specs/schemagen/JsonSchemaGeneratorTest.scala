...
import com.simscale.specs.schemagen.testData._
import com.simscale.specs.schemagen.testData.mixin.{MixinChild1, MixinModule, MixinParent}
import com.simscale.specs.schemagen.testData.polymorphism1.{Child1, Child2, Parent}
import com.simscale.specs.schemagen.testData.polymorphism2.polymorphism1.{Child21, Child22, Parent2}
import com.simscale.specs.schemagen.testData.polymorphism3.{Child31, Child32, Parent3}
import com.simscale.specs.schemagen.testDataScala._
import com.simscale.specs.schemagen.testData_issue_24.EntityWrapper
...
class JsonSchemaGeneratorTest extends FunSuite with Matchers {
  ...
  test("Generate schema for super class annotated with @JsonTypeInfo - use = JsonTypeInfo.Id.CLASS") {
    ...
    // Java
    {
      ...
      assertChild1(schema, "/oneOf", "Child21", typeParamName = "clazz", typeName = "com.simscale.specs.schemagen.testData.polymorphism2.polymorphism1.Child21")
      assertChild2(schema, "/oneOf", "Child22", typeParamName = "clazz", typeName = "com.simscale.specs.schemagen.testData.polymorphism2.polymorphism1.Child22")
    }
  ...
  test("custom serializer not overriding JsonSerializer.acceptJsonFormatVisitor") {
    ...
    assert(schema.asInstanceOf[ObjectNode].fieldNames().asScala.toList == List("$schema")) // Empty schema due to custom serializer
  }
  ...
  test("Preventing polymorphism by using classTypeReMapping") {
    ...
    val _jsonSchemaGenerator = new JsonSchemaGenerator(_objectMapper, config)
...
trait TestData {
  ...
  val manyDates = ManyDates(LocalDateTime.now(), OffsetDateTime.now(), LocalDate.now())
